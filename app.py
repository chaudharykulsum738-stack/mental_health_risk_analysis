...
def page_header(icon, eyebrow, title, subtitle=None):
    st.markdown(
        ...
        <div>
            ...
            {
                f'<p style="color:{COLOR_MUTED};font-size:1rem">{subtitle}</p>'
                if subtitle else ""
            }
        </div>
        """,
...
# ============================================================
# PRIVACY / PSEUDONYMOUS ID
# ============================================================

def hash_user(username):
    """
    Generates a pseudonymous identifier.

    This is NOT equivalent to cryptographic identity protection.
    Production systems should use a proper identity-management service.
    """
    value = str(username).strip().lower()

    salt = os.environ.get(
        "MINDTRACK_HASH_SALT",
        "development-only-change-this-salt",
    )
...
def save_privacy_preferences(
    ...
    conn.execute(
        ...
        (
            ...
            int(consent_research),
            int(pseudonymous_mode),
    ...
    audit_event(
        ...
        "Consent and privacy settings changed.",
    )
...
def show_login_page():
    ...
    st.markdown(
        """
        <div class="mind-card" style="max-width:650px;margin:auto;text-align:center">
            <div style="font-size:60px">🧠</div>
            <h1>MindTrack</h1>
            <p>Wellness intelligence and self-reflection</p>
            <p class="small-muted">
                This application provides wellness signals, not medical diagnoses.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
...
def show_user_badge():
    if (
        ...
        st.sidebar.markdown(
            f"""
            <div class="mind-card">
                👤 <b>{safe_text(st.session_state.current_user)}</b>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def calculate_wellness_score(
    ...
    """
    Wellness score for prototype purposes.

    Higher score = more favorable self-reported indicators.

    This is intentionally described as a wellness score,
    not a clinical severity score.
    """
...
def missing_data_intelligence(
    ...
    if journal_text is not None and not str(
        journal_text
    ).strip():
...
def save_user_history(
    ...
    journal_polarity=None,
    demographic_group=None,
):
    preferences = get_privacy_preferences(
        username
    )
...
def delete_all_entries(username):
    ...
    conn.execute(
        "DELETE FROM user_history WHERE username=?",
        (username,),
    )
    ...
    audit_event(
        username,
        "all_assessments_deleted",
        "All assessment history deleted.",
    )
...
def analyze_text_modality(text):
    ...
    This does NOT diagnose mental illness.
    It extracts a simple sentiment signal and safety-related
...
def analyze_structured_modality(
    stress,
    anxiety,
    sleep,
    exercise,
...
def analyze_uploaded_signal(
    ...
    """
    Optional third modality.

    Accepted:
      CSV
      JSON

    The uploaded file can contain additional contextual
    signals such as:
    ...
    try:
        ...
        if name.endswith(".csv"):
            ...
            return {
                "available": False,
                "signals": {},
                "error": "Only CSV and JSON are supported.",
            }
...
def multimodal_risk_analysis(
    ...
    structured = analyze_structured_modality(
        stress,
        anxiety,
        sleep,
        exercise,
    ...
    text = analyze_text_modality(
        journal_text
    )

    extra = analyze_uploaded_signal(
        uploaded_signal
    )
    ...
    # Text changes the explanation rather than independently
    # diagnosing the person.
    if text["available"]:
        if text["polarity"] < -0.4:
            factors.append(
                "Journal language shows strongly negative sentiment"
            )

        elif text["polarity"] > 0.4:
            factors.append(
                "Journal language shows positive sentiment"
            )

    if extra["available"]:
        factors.append(
            "Additional contextual data was incorporated"
        )
    ...
    if (
        ...
        factors.append(
            "Safety-related language detected; human review is recommended"
        )
...
def predict_risk(
    ...
    if not factors:
        factors.append(
            "No major unfavorable indicators in the submitted check-in"
        )
...
def save_prediction(
    ...
    audit_event(
        username,
        "prediction_created",
        f"Risk={risk_level}; trajectory={trajectory}",
    )
...
def calculate_risk_trajectory(
    ...
    """
    Estimate trajectory from wellness history.

    This is a trend indicator, not a forecast of mental illness.
    """
...
def trajectory_chart(df):
    ...
    fig = px.line(
        ...
        y="wellness_score",
        markers=True,
        title="Wellness Risk Trajectory",
    ...
    fig.add_hline(
        ...
        annotation_text="Higher wellness",
    )
    ...
    fig.add_hline(
        ...
        annotation_text="Lower wellness",
    )
...
def missingness_explanation(
    missing,
):
    explanations = {
        "sleep":
            "Sleep is missing, so sleep-related wellness contribution is unavailable.",
        "stress":
            "Stress is missing, reducing confidence in the current wellness estimate.",
        "anxiety":
            "Anxiety is missing, reducing confidence in the overall assessment.",
        "exercise":
            "Exercise is missing, so activity-related context is unavailable.",
        "mood":
            "Mood is missing, reducing the ability to compare subjective well-being.",
        "journal/text":
            "No journal text was provided, so language-based context is unavailable.",
    }

    return [
        explanations[x]
        for x in missing
        if x in explanations
    ]
...
def fairness_audit(df):
    """
    Research-oriented subgroup audit.

    IMPORTANT:
    Demographic fields should only be collected with appropriate
    consent and governance.

    The application does not use demographic group as an input
    to the wellness score.
    """
...
def create_local_model_update(
    username
):
    """
    Foundation for federated learning.

    Instead of sending raw journal/check-in data,
    the prototype creates a local aggregate update.

    Production federated learning would use:
      - secure aggregation
    ...
    This function does NOT transmit anything.
    """
...
def export_all_user_data(
    ...
    if predictions.empty:
        predictions_user = pd.DataFrame()
    else:
        predictions_user = predictions[
            predictions[
                "username"
            ].astype(str)
            == str(username)
...
def safe_chatbot_response(
    username,
    message
):
    """
    Safe conversational layer.
    ...
    For production, this layer could be connected to a vetted
    LLM through a controlled server-side gateway with:
      safety policy
      moderation
      logging
      privacy filtering
...
def create_factor_bar(
    ...
    fig.add_trace(
        go.Bar(
            ...
            x=categories,
            y=healthy_targets,
            marker_color=COLOR_SLATE,
        )
    )

    fig.update_layout(
        barmode="group",
        title="Current Factors vs Reference Targets",
    )

    return style_plot(fig)


def create_mood_calendar(
    df,
    weeks=12,
):
    if df.empty:
    ...
    for offset in range(
        ...
        hovertext[
            ...
            f"<br>"
            f"{'Mood: %.1f' % val if not np.isnan(val) else 'No entry'}"
        )
    ...
    fig.update_layout(
        title=f"Mood Calendar — Last {weeks} Weeks"
    )
...
def generate_pdf_report(
    ...
    brand_color = rl_colors.HexColor(
        COLOR_PRIMARY
    )

    ink_color = rl_colors.HexColor(
        COLOR_INK
    )
    ...
    story.append(
        Paragraph(
            f"Wellness signal: {wellness_score}/100",
            info_style,
        )
    )
...
st.sidebar.markdown(
    ...
    ">
        ...
        <h2 style="margin:0">MindTrack</h2>
        <p class="small-muted">
...
if page == "🏠 Home":
    ...
    page_header(
        ...
        "A privacy-aware self-reflection workspace.",
    )
    ...
    if not history.empty:
        ...
        if not mine.empty:
            ...
            m2.metric(
                "Avg Wellness",
                f"{mine['wellness_score'].mean():.0f}/100",
            )
    ...
    tips = [
        ...
        "Try two minutes of slow breathing.",
        "Drink some water.",
...
# ============================================================
    ...
    page_header(
        ...
        "A short self-reflection check-in.",
    )
    ...
    c1.metric(
        "Data completeness",
        f"{missing['completeness']:.0f}%",
    )

    c2.metric(
        "Estimated confidence",
        f"{missing['confidence']:.0%}",
    )
    ...
    preview = calculate_wellness_score(
        stress_level,
        sleep_hours,
        anxiety_level,
        exercise_minutes,
    ...
    if st.button(
        ...
        saved = save_user_history(
            ...
            journal_polarity=polarity,
            demographic_group=(
                demographic_group.strip()
                or None
            ),
        )
...
# ============================================================
    ...
    page_header(
        ...
        "WELLNESS SIGNAL",
        "Risk Analysis",
        "A transparent self-reflection signal based on submitted information.",
    )
    ...
    if "assessment_data" not in st.session_state:
        ...
        if risk == "Low":
            st.success(
                "🎯 Current wellness indicators fall in the Low band."
            )
        elif risk == "Medium":
            st.warning(
                "⚠️ Current wellness indicators fall in the Medium band."
            )
        else:
            st.error(
                "🚨 Current wellness indicators fall in the High band. "
        ...
        modality_cols[0].metric(
            "Structured data",
            "✓",
        )

        modality_cols[1].metric(
            "Journal text",
            "✓"
            if result["text"][
                "available"
        ...
        st.write(
            f"**Direction:** {trajectory['trajectory']}"
        )

        st.write(
            f"**Change:** {trajectory['change']:+.1f} wellness points"
        )

        st.write(
            f"**Trajectory confidence:** {trajectory['confidence']:.0%}"
        )
        ...
        if (
            ...
            st.warning(
                "This result meets the prototype's human-review criteria."
            )
            ...
            if st.button(
                ...
                if not predictions.empty:
                    ...
                    if not latest.empty:
                        ...
                        st.success(
                            "Added to the human-review queue."
                        )
...
# ============================================================
    ...
    page_header(
        ...
        "Risk Trajectory",
        "Look at changes across multiple check-ins rather than a single score.",
    )
    ...
    if history.empty:
        st.info(
            "Complete several assessments to build a trajectory."
        )
        ...
        if mine.empty:
            ...
            st.info(
                "No history for your account yet."
            )
